...
ALTER TABLE financial_contributions ADD CONSTRAINT fk_project FOREIGN KEY (project_id) REFERENCES projects(id);
  ...
  app.get("/api/users/template", requireAuth, async (req, res) => {
    try {
      ...
      const templateData = [
        ...
        ["Samir", "Halilović", "samir.halilovic", "password123", "samir@example.com", "+387 61 123 456", "Ulica Maršala Tita 15", "71000", "Sarajevo", "2024-01-15", "aktivan"],
        ["Samira", "Halilović", "samira.halilovic", "password123", "samira@example.com", "+387 62 234 567", "Koning Tvrtka 22", "72000", "Zenica", "2023-06-20", "aktivan"]
      ];
  ...
  app.post("/api/important-dates", requireAdmin, async (req, res) => {
    try {
      ...
      console.error('Request body:', req.body);
      res.status(500).json({ message: "Failed to create important date", details: error?.message });
    }
});
