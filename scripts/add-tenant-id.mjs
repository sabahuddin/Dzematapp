    ...
    if (routeMatch) {
      ...
      while (i < lines.length) {
        ...
        if (currentLine.match(/async\s*\(req,\s*res\)\s*=>\s*\{/)) {
          const routeHandlerIndent = currentLine.length - currentLine.trimStart().length;
...
function updateStorageCall(line, isLoginRoute = false) {
  ...
  const patterns = [
    ...
    [/storage\.updateRequestStatus\(([^,]+),\s*([^,]+),\s*([^,]+),\s*([^,)]+)\)/, 'storage.updateRequestStatus($1, tenantId, $2, $3, $4)'],
