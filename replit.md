...
## Bugfixes
- **User Profile Updates**: Fixed authorization logic to allow users to update their own profiles (name, email, phone, occupation, address, username, password) while restricting sensitive fields (roles, points, membership status) to admins only. Previously, all profile updates required admin privileges.
